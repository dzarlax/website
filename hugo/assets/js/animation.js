...
function setupSmoothScrolling() {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            // Only intercept in-page anchors. Let real URLs (e.g. /articles/)
            // navigate normally — `querySelector('/articles/')` would throw,
            // but more importantly we don't want preventDefault on them.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Default to the first link only if nothing else is already marked
    // active — Hugo blog pages server-render `.active` on the right item
    // (e.g. Articles), and we don't want to override that.
    const navLinks = document.querySelectorAll('nav a');
    const hasActive = Array.from(navLinks).some(a => a.classList.contains('active'));
    if (navLinks.length > 0 && !hasActive) {
